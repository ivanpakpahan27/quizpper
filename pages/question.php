...
if ($res4 == true) {
    ...
    $student_subject_id = $row4['subject_id'];
    $full_name = $first_name . ' ' . $last_name;
}
//get total time and total no. of questions based on the subject of the student
$tbl_name_qns = 'tbl_subject';
$where_qns = "subject_id='$student_subject_id'";
$query_qns = $obj->select_data($tbl_name_qns, $where_qns);
$res_qns = $obj->execute_query($conn, $query_qns);
if ($res_qns == true) {
    $row_qns = $obj->fetch_data($res_qns);
    $subject_name = $row_qns['subject_name'];
...
<div class="main">
    <div class="content">
        ...
        Subject: <span class="heavy"><?php echo $subject_name; ?></span>&nbsp;&nbsp;
            ...
            <div class="welcome">
                <div class="ques">
                    ...
                    if ($sn <= $total_english) {
                        ...
                        $where = "is_active='yes' && category='English' && subject_id='" . $student_subject_id . "' && question_id NOT IN (" . $_SESSION['all_qns'] . ")";
                    } else {
                        //New Query FOR MATHS
                        $where = "is_active='yes' && category='Math' && subject_id='" . $student_subject_id . "' && question_id NOT IN (" . $_SESSION['all_qns'] . ")";
                    }
                    //Get Maths Questions Only
                    //New Query
                    //$where="is_active='yes' && subject_id='".$student_subject_id."' && question_id NOT IN (".$_SESSION['all_qns'].")";
                    //Old Query
                    //$where="is_active='yes'";
                    $limit = 1;
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        if (isset($_POST['answer'])) {
                            $student_answer = $obj->sanitize($conn, $_POST['answer']);
                        } else {
                            $student_answer = 0;
                        }
